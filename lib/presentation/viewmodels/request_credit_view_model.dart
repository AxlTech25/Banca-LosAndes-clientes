...
class RequestCreditViewModel extends ChangeNotifier {
  ...
  String? get clienteUbicacion => _cliente?['direccion']?.toString();
  double? get clienteIngresos =>
      _parseDouble(_cliente?['ingresos_estimados']);
  double? get clienteGastosMensuales =>
      _parseDouble(_cliente?['gastos_mensuales']);

  String get clienteNombreCompleto {
    final nombres = _cliente?['nombres']?.toString().trim() ?? '';
    final apellidos = _cliente?['apellidos']?.toString().trim() ?? '';
    return [nombres, apellidos].where((part) => part.isNotEmpty).join(' ');
  }

  bool get perfilNegocioCompleto {
    final tipo = clienteTipoNegocio?.trim() ?? '';
    final nombre = clienteNombreNegocio?.trim() ?? '';
    final ubicacion = clienteUbicacion?.trim() ?? '';
    final antiguedad = clienteAntiguedadMeses;
    final ingresos = clienteIngresos;
    final gastos = clienteGastosMensuales;
  ...
  NuevaSolicitudInput? buildSolicitudInput({
    required double montoSolicitado,
    required int plazoMeses,
    required String destinoCredito,
    required String garantia,
    required bool conSeguroDesgravamen,
    double? teaReferencial,
    double? cuotaEstimada,
  }) {
    if (!perfilNegocioCompleto) return null;

    return NuevaSolicitudInput(
      tipoNegocio: clienteTipoNegocio!,
      nombreNegocio: clienteNombreNegocio!,
      ubicacionNegocio: clienteUbicacion!,
      antiguedadMeses: clienteAntiguedadMeses!,
      ingresosEstimados: clienteIngresos!,
      gastosMensuales: clienteGastosMensuales!,
      montoSolicitado: montoSolicitado,
      plazoMeses: plazoMeses,
      destinoCredito: destinoCredito,
      garantia: garantia,
      conSeguroDesgravamen: conSeguroDesgravamen,
      teaReferencial: teaReferencial,
